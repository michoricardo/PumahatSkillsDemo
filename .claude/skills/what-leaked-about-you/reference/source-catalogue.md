...
## Reading IntelX responses

Lessons from real queries that change how a result should be worded:

- **`altterm` means the term was widened.** For an email, IntelX may also match
  the bare domain (`gmail.com`, `hotmail.com`). Hits can then be public web pages
  or multi-million-line combolists that merely contain the domain. A hit proves
  the domain appears, not that the address does.
- **Hits are file fragments, not records.** Large dumps are split into parts of
  a few megabytes ("Part 129 of 218"), and dates are ingestion dates. Sixteen
  hits are not sixteen breaches: deduplicate and name distinct sources.
- **`softselectorwarning: true` with zero records is inconclusive.** Bare digit
  strings and ambiguous terms may not be run as a selector at all. Try the
  formats from `scripts/variants.py`, or use the vendor's web interface.
- **Result status.** As documented by IntelX (verify for your plan): 0 results,
  1 no more results, 2 search id not found, 3 not ready yet, 4 error. Only 0 and 1
  with an empty list mean "nothing indexed".
- **Free plans still return leak buckets.** Do not assume a free key is limited
  to public web results, and do not assume it covers everything.
- **Metadata first.** Bucket names (`web.public.*`, `darknet.tor`, `leaks.logs`,
  `leaks.private.general`) separate public pages from stealer logs and dumps.
  Opening a fragment exposes third parties' credentials, so filter locally for
  the exact selector and discard the rest.
