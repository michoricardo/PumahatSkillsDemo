...
description: >-
  ...
  auditing your own leaked personal data. Also generates search variants (email, username,
  phone) and plans pivots from a selector the user owns. Applies to incident response and
  account-takeover triage, executive and VIP protection, pre-employment and vendor risk
  screening, and personal privacy audits. Reference at useosint.com/skills/what-leaked-about-you.
...
Before any lookup, establish that the selector is the user's own, or that its
owner has authorized the work. If neither is clear, ask once and stop until it
is. A mock or invented selector needs no gate, but say plainly that nothing was
queried. Scope applies to derived selectors too: see below.
...
## Derived selectors and pivot planning

One selector rarely tells the whole story: the same mailbox is indexed as
`john.smith@gmail.com`, `johnsmith@gmail.com` and `johnsmith@googlemail.com`, and
a phone number appears as `+52…`, `52…`, a spaced form, or a legacy mobile
format. A search that ignores this reports a false negative. Plan the variants
before spending any queries.

Generate them with `scripts/variants.py` (no network access):

```
python3 scripts/variants.py email    <address>
python3 scripts/variants.py phone    <number> [--country MX|US]
python3 scripts/variants.py username <handle>
```

Each variant carries a kind, and the kind decides what you may do with it:

| Kind | Meaning | Rule |
|---|---|---|
| `equivalent` | Provably the same mailbox or number: case, `+tag`, Gmail dots and `googlemail.com`, number formats | Search once the scope gate is passed |
| `derived` | A relative: same handle on another domain (`outlook.com`, `live.com`), a separator change, a trimmed suffix | Never search on your own. List it and let the user confirm it is theirs. It may belong to a stranger |

Do not enumerate guesses at addresses the user has not named. Producing
candidates for someone else's mailbox is discovery against a third party, not
auditing.

**Budget.** Keyed sources meter queries. Before running more than three
searches, show the planned variants ranked by priority (1 is the canonical form
the source most likely indexed) and get a go-ahead. `scripts/intelx_search.py`
enforces a `--max-searches` cap and returns metadata only. Its verdicts are
`HITS`, `NONE` and `INCONCLUSIVE`. Report `INCONCLUSIVE` as such: a soft-selector
warning or a status 2 result means the query did not run properly, which is not
the same as "nothing found".

**Pivot plan.** Close every self-audit with a table of what the results opened up,
in priority order, and mark which items are done, which need the user's
confirmation, and which are blocked by a missing skill or key:
...
| Breach names from Have I Been Pwned | curated attribution and dates | the user runs the web lookup |
